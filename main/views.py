...
def tail_log_file(request, instance_id):
    out = subprocess.run(["tail", f"crawlers/log/{instance_id}.out", "-n", "15"], stdout=subprocess.PIPE).stdout
    err = subprocess.run(["tail", f"crawlers/log/{instance_id}_error.out", "-n", "15"], stdout=subprocess.PIPE).stdout
    return JsonResponse({
